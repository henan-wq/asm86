...
9. 将`number`的汇编地址传送到寄存器BX，将立即数4传送给寄存器si
    ...
    9. 在显示了各个数位后，再显示一个字符“D”，目的地址是ES:DI给出的，源操作数是立即数`0x0744`，高字节`0x07`是黑底白字，低字节`0x44`是字符“D”的ASCII码。
    10. `$`是标记，把它看作是一个隐藏在当前行行首的标号。`$`代表的是当前行，只是不需要声明就能立刻使用。`jmp near $`意思是转移到当前指令继续执行。
    11. `$$`是代表当前汇编节（段）的起始汇编地址。因为`c06_mbr`没有定义节或段，就默认地自成一个汇编段，而且起始汇编地址是0（程序起始处），第8章会谈到代码段和数据段……